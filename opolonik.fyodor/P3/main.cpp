...
void task1_processMatrix(int** matrix, int rows, int cols) {
    // Количество слоев — это половина от меньшей стороны (если нечётно — ещё один слой)
    int layers = std::min(rows, cols) / 2 + (std::min(rows, cols) % 2);

    for (int layer = 0; layer < layers; ++layer) {
        // На каждом слое прибавляем число слоя (1 на внешнем, 2 на следующем и т.д.)
        ...
        for (int j = left; j <= right; ++j) matrix[top][j] += increment;

        // Нижняя строка, если она не совпала с верхней
        if (top != bottom)
            for (int j = left; j <= right; ++j) matrix[bottom][j] += increment;

        // Левая вертикаль слоя
        for (int i = top + 1; i < bottom; ++i) matrix[i][left] += increment;

        // Правая вертикаль, если не совпала с левой
        if (left != right)
            for (int i = top + 1; i < bottom; ++i) matrix[i][right] += increment;
    }
}
...
int task2_countNonZeroDiagonals(int** matrix, int rows, int cols) {
    ...
    for (int startCol = 0; startCol < cols; ++startCol) {
        ...
        while (i < rows && j < cols) {
            if (matrix[i][j] == 0) { ok = false; break; }
            i++; j++;
        }
        if (ok) count++;
    }
...
bool readMatrix(int** matrix, int rows, int cols, std::ifstream& fin) {
    for (int i = 0; i < rows; ++i)
        for (int j = 0; j < cols; ++j)
            if (!(fin >> matrix[i][j])) return false;
    return true;
}
...
int main(int argc, char* argv[]) {
    ...
    int rows, cols;
    if (!(fin >> rows >> cols)) { std::cerr << "Invalid matrix format\n"; return 2; }
    if (rows < 0 || cols < 0) { std::cerr << "Invalid matrix dimensions\n"; return 2; }
    ...
    if (num == 1) {
        ...
        if (rows * cols > 10000) { std::cerr << "Matrix too large for static mode\n"; return 2; }

        static int buffer[100][100];
        matrix = new int*[rows];
        for (int i = 0; i < rows; ++i) matrix[i] = buffer[i];
    } 
    else { // Динамическое выделение
        matrix = new (std::nothrow) int*[rows];
        if (!matrix) { std::cerr << "Memory allocation error\n"; return 2; }

        for (int i = 0; i < rows; ++i) {
            matrix[i] = new (std::nothrow) int[cols];
            if (!matrix[i]) { std::cerr << "Memory allocation error\n"; return 2; }
        }
    }

    // Читаем матрицу
    if (!readMatrix(matrix, rows, cols, fin)) { std::cerr << "Invalid matrix content\n"; return 2; }
