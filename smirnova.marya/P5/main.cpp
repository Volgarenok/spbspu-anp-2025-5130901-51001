...
inline void moveToPoint(point_t &center, const point_t &p) { center = p; }

inline rectangle_t makeFrameRect(const double w, const double h,
                                 const point_t &center) noexcept {
  return {w, h, center};
}

inline point_t getCentroid(const point_t &a, const point_t &b,
...
void Rectangle::move(const point_t &p) { moveToPoint(center_, p); }
...
Square::Square(const double side, const point_t &center)
...
void Square::move(const point_t &p) { moveToPoint(center_, p); }
...
rectangle_t Triangle::getFrameRect() const noexcept {
  ...
  return makeFrameRect(maxX - minX, maxY - minY,
...
rectangle_t getTotalFrameRect(Shape *shapes[], int count) noexcept {
  ...
  for (int i = 1; i < count; ++i) {
    ...
    double minX =
        std::min(total.pos.x - total.width / 2, f.pos.x - f.width / 2);
    double maxX =
        std::max(total.pos.x + total.width / 2, f.pos.x + f.width / 2);
    double minY =
        std::min(total.pos.y - total.height / 2, f.pos.y - f.height / 2);
    double maxY =
        std::max(total.pos.y + total.height / 2, f.pos.y + f.height / 2);
...
int main() {
  ...
  try {
    ...
    std::cout << "Overall bounding box: width = " << totalFrame.width
              << ", height = " << totalFrame.height << ", center = ("
              << totalFrame.pos.x << ", " << totalFrame.pos.y << ")\n";
