...
rectangle_t Rectangle::getFrameRect()
    const noexcept
{
    return makeFrameRect(width_, height_,
                         center_);
}
                              ...
                              int count) noexcept
{
    ...
    for (int i = 1; i < count; ++i)
    {
        ...
        double minX = std::min(
            total.pos.x - total.width / 2,
            f.pos.x - f.width / 2);
        double maxX = std::max(
            total.pos.x + total.width / 2,
            f.pos.x + f.width / 2);
        double minY = std::min(
            total.pos.y - total.height / 2,
            f.pos.y - f.height / 2);
        double maxY = std::max(
            total.pos.y + total.height / 2,
            f.pos.y + f.height / 2);
    ...
    try
    {
        ...
        for (int i = 0; i < 3; ++i)
        {
            rectangle_t frame =
                shapes[i]->getFrameRect();
            double dx =
                scalePoint.x - frame.pos.x;
            double dy =
                scalePoint.y - frame.pos.y;
            shapes[i]->move(dx, dy);
            shapes[i]->scale(k);
            shapes[i]->move(-dx, -dy);
        }
