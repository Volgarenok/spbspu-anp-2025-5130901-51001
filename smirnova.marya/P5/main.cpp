...
inline void moveCenter(point_t& center,
                       const double dx,
                       const double dy)
{
  center.x += dx;
...
inline void moveToPoint(point_t& center,
                        const point_t& p)
{
  center = p;
}

inline rectangle_t makeFrameRect(
...
void Rectangle::move(const double dx,
                     const double dy)
{
...
void Square::move(const double dx,
                  const double dy)
{
  moveCenter(center_, dx, dy);
}
...
{
 public:
  Triangle(const point_t a, const point_t b,
           const point_t c);
  double getArea() const noexcept override;
  rectangle_t getFrameRect()
      const noexcept override;
...
void Triangle::move(const double dx,
                    const double dy)
{
  moveCenter(a_, dx, dy);
  moveCenter(b_, dx, dy);
...
rectangle_t getTotalFrameRect(Shape* shapes[],
                              int count) noexcept
{
  if (count <= 0)
  {
  ...
  for (int i = 1; i < count; ++i)
  {
    ...
    double minX =
        std::min(total.pos.x - total.width / 2,
                 f.pos.x - f.width / 2);
    double maxX =
        std::max(total.pos.x + total.width / 2,
                 f.pos.x + f.width / 2);
    double minY =
        std::min(total.pos.y - total.height / 2,
                 f.pos.y - f.height / 2);
    double maxY =
        std::max(total.pos.y + total.height / 2,
                 f.pos.y + f.height / 2);
    total.width = maxX - minX;
    total.height = maxY - minY;
    total.pos = {(minX + maxX) / 2.0,
                 (minY + maxY) / 2.0};
  }
  return total;
}
...
int main()
{
  ...
  try
  {
    ...
    Triangle tri({0.0, 0.0}, {4.0, 0.0},
                 {2.0, 3.0});
    Shape* shapes[3] = {&rect, &tri, &sq};
    const char* names[3] = {"Rectangle",
                            "Triangle", "Square"};
    ...
    for (int i = 0; i < 3; ++i)
    {
      ...
      std::cout << names[i]
                << ": square = " << area << "\n";
      std::cout << "Bounding rectangle: width = "
                << frame.width
                << ", height = " << frame.height
                << ", center = (" << frame.pos.x
                << ", " << frame.pos.y << ")\n";
    ...
    std::cout << "Total area: " << totalArea
              << "\n";
    rectangle_t totalFrame =
        getTotalFrameRect(shapes, 3);
    std::cout << "Overall bounding box: width = "
              << totalFrame.width << ", height = "
              << totalFrame.height
              << ", center = ("
              << totalFrame.pos.x << ", "
              << totalFrame.pos.y << ")\n";
